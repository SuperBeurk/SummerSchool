...
# 11 "class/../libraries/lcd_highlevel.h"
typedef struct
{
    uint8_t width;
    uint16_t offset;
...
# 1 "class/../libraries/lcd_lowlevel.h" 1
# 252 "class/../libraries/lcd_lowlevel.h"
void LCD_PowerOff(void);
# 261 "class/../libraries/lcd_lowlevel.h"
void LCD_Lowlevel_Init(void);
# 271 "class/../libraries/lcd_lowlevel.h"
...
RESTART_SPI:
  ...
  TscWrite (0x40, 1, 0x01| 0x02);

  TscWrite (0x04, 1, 0x08 |
                                   0x04);

  TscWrite (0x0A, 1, 0x01 |
                                0x02 |
                                0x04);

  TscWrite (0x20, 1, ((0x6 & 0x7)
                                   << 4 )|
                                   0x8);
  ...
  TscWrite (0x4A, 1, 0x01);

  TscWrite (0x4B, 1, 0x01);

  TscWrite (0x4B, 1, 0x00);

  TscWrite (0x56, 1,
                 (0x07 * 0x7) << 0);

  TscWrite (0x58, 1, 0x01);


  TscWrite (0x40, 1,0x01 |0x02);

  TscWrite (0x0B, 1, 0xFF);

  TscWrite (0x09, 1, 0x02|
                   0x01);
  INT1IE = 1;
  INTEDG1 = 0;
}
...
void TscUpdate(void)
{
  ...
  TscRead(0x0B, 1, &intStat);
  if(intStat & 0x01)
  {
    TscRead(0x40, 1, tempTbl);
    if(tempTbl[0] & 0x80)
    {
    }
    else
  ...
  TscRead(0x4B, 1, tempTbl);
  if((intStat & 0x02)||(tempTbl[0] == 0x10))
  {
    tsc.pressed = 1;
    tsc.released = 0;
    ...
    TscRead(0x4D, 2, tempTbl);
    xPos = ((uint16_t)tempTbl[0] << 8) | tempTbl[1];
    TscRead(0x4F, 2, tempTbl);
    yPos = ((uint16_t)tempTbl[0] << 8) | tempTbl[1];

    tsc.posY = ((yPos *(320 -1)) / (280L -3800L))-(((320 -1) * 3800L) / (280L -3800L));
    tsc.posX = ((xPos *(240 -1)) / (3800L -380L))-(((240 -1) * 380L) / (3800L -380L));
